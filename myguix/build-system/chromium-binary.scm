...
(define-module (myguix build-system chromium-binary)
  ...
  #:export (%chromium-binary-build-system-modules lower chromium-binary-build
...
(define %chromium-binary-build-system-modules
  ...
  `((myguix build chromium-binary-build-system)
    (myguix build utils)
...
(define (build-patchelf-plan wrapper-plan inputs)
  #~(let ((patchelf-inputs (list #$@(map car inputs))))
      (map (lambda (file)
             ...
             (if (list? file)
                 (cons (car file)
                       (list (append patchelf-inputs
                                     (cadr file))))
                 (cons file
                       (list patchelf-inputs))))
           #$wrapper-plan)))
...
(define* (chromium-binary-build name
                                inputs
                                #:key guile
                                source
                                wrap-inputs
...
(define chromium-binary-build-system
  (build-system (name 'chromium-binary)
