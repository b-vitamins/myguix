...
(define-module (myguix build bazel-build-system)
...
`bazel build --nobuild'.  Restore symlinks after unpacking."
  ...
  (for-each (lambda (file)
              (let ((new-target (string-replace-substring (readlink file)
                                                          "GUIX_BUILD_TOP"
                                                          (getenv
                                                           "NIX_BUILD_TOP"))))
                (delete-file file)
...
(define %standard-phases
  (modify-phases gnu:%standard-phases
    (delete 'bootstrap)
    (add-after 'unpack 'unpack-vendored-inputs
      unpack-vendored-inputs)
    (delete 'configure)
    (replace 'build
      build)
    (delete 'install)))

(define* (bazel-build #:key inputs
