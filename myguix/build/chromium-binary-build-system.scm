...
(define-module (myguix build chromium-binary-build-system)
  ...
  #:export (%standard-phases chromium-binary-build))
...
(define* (install-wrapper #:key inputs outputs #:allow-other-keys)
  (let* ((output (assoc-ref outputs "out"))
         ...
         (bin-directories (search-path-as-list '("bin" "sbin" "libexec")
                                               wrap-inputs)))
    (for-each (lambda (exe)
                (display (string-append "Wrapping " exe "\n"))
                (wrap-program exe
                  `("FONTCONFIG_PATH" ":" prefix
                    (,(string-join (list (string-append fontconfig-minimal
                                                        "/etc/fonts") output)
                                   ":")))
                  `("PATH" ":" prefix
                    (,(string-join (append bin-directories
                                           (list bin)) ":")))
...
(define* (chromium-binary-build #:key inputs
