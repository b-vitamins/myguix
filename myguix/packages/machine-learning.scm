(define-module (myguix packages machine-learning)
  ...
  #:use-module ((gnu packages machine-learning)
                #:hide (python-pytorch python-torchvision tensorflow xnnpack))
    ...
    (package
      ...
      (source
       (origin
         ...
         (file-name (git-file-name name version))
         (sha256
          (base32 "0j4smj8yaxw6r14caqn0dgb8j8mnsq34zp8ixxn11zmgbab306ly"))
         (modules '((guix build utils)
                    (ice-9 ftw)
                    (ice-9 textual-ports)
                    (srfi srfi-26)))
                                 ...
                                 (for-each (lambda (name)
                                             (let ((path (string-append dir
                                               ...
                                               (when (call-with-input-file path
                                                       (lambda (port)
                                                         (string-contains (get-string-all
                                                                           port)
                                                          "Auto-generated file")))
                                                 (delete-file path))))
                                           (scandir dir
        ...
        #~(modify-phases %standard-phases
            ...
            (add-after 'patch-source-shebangs 'generate-files
              (lambda _
                ...
                (invoke "python3"
                        "tools/generate-lut-norm-test.py"
                        "--spec"
                        "test/u8-lut32norm.yaml"
                        "--output"
                        "test/u8-lut32norm.cc")
                (invoke "python3"
                        "tools/generate-gemm-test.py"
                        "--spec"
                        "test/qd8-f16-qb4w-gemm-minmax.yaml"
                        "--output-test"
                        "test/qd8-f16-qb4w-gemm-minmax.cc")
                (invoke "python3"
                        "tools/generate-gemm-test.py"
                        "--spec"
                        "test/qd8-f32-qb4w-gemm-minmax.yaml"
                        "--output-test"
      ...
      (inputs (list clog
                    ...
                    fp16
                    psimd))
      (native-inputs (append (if (target-riscv64?)
...
TensorFlow.js, PyTorch, and MediaPipe.")
      (supported-systems '("armhf-linux" "aarch64-linux" "riscv64-linux"
      ...
      #~(modify-phases %standard-phases
          ...
          (add-before 'build 'use-system-libraries
            (lambda _
              ...
              ;; Update sympy version constraint for PyTorch 2.7
              (substitute* '("requirements.txt" "setup.py")
                (("sympy==1\\.13\\.1")
                 "sympy>=1.13.1"))))
          (add-after 'use-system-libraries 'skip-nccl-call
            (lambda _
              ;; Comment-out the *single* invocation of `checkout_nccl()` in build_pytorch().
