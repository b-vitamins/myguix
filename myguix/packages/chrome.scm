...
(define-module (myguix packages chrome)
...
(define-public (make-google-chrome repo version hash)
  (let* ((name (string-append "google-chrome-" repo))
         (appname (if (string=? repo "stable") "chrome"
    ...
    (package
      ...
      (arguments
       (list
        ...
        #:substitutable? #f
        #:wrapper-plan
        #~(let ((path (string-append "opt/google/"
                                     #$appname "/")))
            (map (lambda (file)
            ...
            (add-before 'install 'patch-assets
              ...
              (lambda _
                (let* ((bin (string-append #$output "/bin"))
                       ...
                       (opt "./opt")
                       (usr/share "./usr/share")
                       (old-exe (string-append "/opt/google/"
                                               #$appname "/google-"
                                               #$appname))
                       (exe (string-append bin "/google-"
                                           #$appname)))
                  ;; This allows us to override CHROME_WRAPPER later.
                  (substitute* (string-append opt "/google/"
                                              #$appname "/google-"
                                              #$appname)
                    (("CHROME_WRAPPER")
                     "WRAPPER"))
              ...
              (lambda _
                (let* ((bin (string-append #$output "/bin"))
                       (exe (string-append bin "/google-"
                                           #$appname))
                       (share (string-append #$output "/share"))
                       (chrome-target (string-append share "/google/"
                                                     #$appname "/google-"
                                                     #$appname)))
                  (mkdir-p bin)
...
(define-public google-chrome-stable
  (make-google-chrome "stable" "139.0.7258.127"
                      "1zq0xy7n97z2vklagwh25wmgbhzfrl94jjnvdvr16a7v334zz17k"))

(define-public google-chrome-beta
  (make-google-chrome "beta" "140.0.7339.16"
                      "0zyq0sigr5pxhylp3mr45f4i3xjsbj2rmgvymgkb407ffmlcywn2"))

(define-public google-chrome-unstable
  (make-google-chrome "unstable" "141.0.7354.0"
                      "0b5n3flblzbfjmvkyj1lxicx48jzx47l5rba3i3j3p2n4p5hvk75"))

(define-public google-chrome-canary
  (make-google-chrome "canary" "141.0.7357.0"
                      "0hc4k4nyxrvc07dmg5cbr795d2f56dff7br905pj9cxhg2ha7c5j"))
