...
(define-module (myguix packages firmware)
    ...
    (package
      ...
      (source
       (origin
         ...
         (sha256
          (base32 "19xmkdvlkczc6zgigy8jdbgnp37i6pc03m2cm3gilvzg8m7v18ad"))))
      ...
      (description
       "This package provides binary firmware for the 0V5640 sensor
in the PinePhone.")
    ...
    (package
      ...
      (source
       (origin
         ...
         (file-name (git-file-name name version))
         (sha256
          (base32 "1s52rpikw0gysph5lq7vr6b3nsxczg4ikgil9zdgmcknjnxk9kbv"))))
      (build-system copy-build-system)
      (arguments
       `(#:substitutable? #f
         #:install-plan '( ;Bluetooth firmware
                          ...
                          ("brcmfmac43456-sdio.clm_blob" "/lib/firmware/brcm/")
...
(define-public bluez-firmware
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append "http://bluez.sf.net/download/" name "-" version
                           ".tar.gz"))
       (sha256
        (base32 "1v4yv6gvlvvwslpb0lj1nsp4r900zxpvxz2ab0sbvimbiw8rw4dn"))))
...
(define-public facetimehd-firmware
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append "https://updates.cdn-apple.com/2019/cert"
             "/041-88431-20191011-e7ee7d98-2878-4cd9-bc0a-d98b3a1e24b1"
             "/OSXUpd10.11.5.dmg"))
       (sha256
        (base32 "009kfk1nrrialgp69c5smzgbmd5xpvk35xmqr2fzb15h6pp33ka6"))))
    (build-system copy-build-system)
    (arguments
     (list
      #:substitutable? #f
                         ...
                         (let ((firmware-in (string-append
                           ...
                           (let ((dmg-subset "dmg-subset"))
                             (#$dump-file-chunk
                              ...
                              #$dmg-subset-size
                              #$dmg-subset-offset)
                             (system (string-join (list (search-input-file
                                                         inputs "/bin/xz")
                                                        "--decompress"
                                                        "--stdout"
                                                        dmg-subset
                           ...
                           (let ((firmware-out "firmware.bin.gz"))
                             (#$dump-file-chunk
                              firmware-in firmware-out
                              #$firmware-size
                              #$firmware-offset)
                             (invoke (search-input-file inputs "/bin/gzip")
                                     "--decompress"
                                     (string-append firmware-out))))))))))
    (native-inputs (list cpio gzip xz))
    (synopsis "Firmware for the FacetimeHD (Broadcom 1570) PCIe webcam")
    (description
     "Firmware for the FacetimeHD webcam.  See
  ...
  (package
    ...
    (source
     (origin
       ...
       (uri (string-append "https://download.info.apple.com/Mac_OS_X"
             "/031-30890-20150812-ea191174-4130-11e5-a125-930911ba098f"
             "/bootcamp" version ".zip"))
       (sha256
        (base32 "07jbh6d0djcvcgj5hhkkw7d6mvcl228yb8rp0a2qqw20ya72rpjf"))))
    (build-system copy-build-system)
    (arguments
     (list
      ...
      #:phases (let ((calibration-files '(("1771_01XX.dat" 19040 1644880)
                 ...
                 #~(modify-phases %standard-phases
                     (add-before 'install 'extract
                       (lambda* (#:key inputs #:allow-other-keys)
                         (invoke (search-input-file inputs "/bin/unrar") "x"
                                 "BootCamp/Drivers/Apple/AppleCamera64.exe")
                         (for-each (lambda (spec)
                                     (apply #$dump-file-chunk
                                            "AppleCamera.sys" spec))
                                   '#$calibration-files)))))))
    (native-inputs (list unrar unzip))
    (synopsis
     "Calibration files for the FacetimeHD (Broadcom 1570) PCIe webcam")
    (description
     "Calibration files for the FacetimeHD webcam.  These are
