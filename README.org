...
Available driver streams:

- =nvda= tracks the 580 production branch and is the default.
- =nvda-590= tracks the 590 new feature branch.
- =nvda-595= tracks the 595 production branch.
- =nvdb= tracks the beta branch.

Per-driver kernel coverage in myguix follows the package definitions carried here. Hardware reports are still welcome.

| Driver     | linux-7.0 | linux-6.19 | linux-6.18 | linux-6.12 | linux-6.6 | linux-6.1 |
|------------+-----------+------------+------------+------------+-----------+-----------|
| =nvdb=     | yes*      | yes        | yes        | yes        | yes       | yes       |
| =nvda-595= | yes*      | yes        | yes        | yes        | yes       | yes       |
| =nvda-590= | yes*      | yes        | yes        | yes        | yes       | yes       |
| =nvda=     | yes*      | yes        | yes        | yes        | yes       | yes       |

- =linux-7.0= is defined in this channel, but it requires a Guix base that already provides =linux-libre-7.0=.
...
Borg backup services for Guix System and Guix Home, with Shepherd timers and a
=borg-guix= helper for manual runs.
         ...
         (borg-backup-configuration
          (jobs
           (list
            (borg-backup-job
             (name "rootfs")
             (repository "ssh://backup@example.org/~/borg/rootfs")
             (passphrase-file "/etc/borg/rootfs-passphrase")
             (schedule "0 3 * * *")
             (check-schedule "30 4 * * 0")
             (paths '("/etc" "/home"))
             (exclude-patterns '("sh:**/.cache/**"
                                 "sh:**/node_modules/**"))
             (prune-keep-daily 7)
             (prune-keep-weekly 4)
...
#+BEGIN_SRC scheme
(use-modules (myguix home services backup))

(service home-borg-backup-service-type
         (borg-backup-configuration
          (jobs
           (list
            (borg-backup-job
             (name "documents")
             (repository "ssh://backup@example.org/~/borg/documents")
             (passcommand "secret-tool lookup borg documents")
             (schedule "0 2 * * *")
             (paths (list #~(string-append (getenv "HOME") "/Documents")
                          #~(string-append (getenv "HOME") "/Projects")))
             (prune-keep-daily 14)
             (prune-keep-monthly 12))))))
#+END_SRC

Once the service is installed you can manually run =borg-guix backup JOB=,
=borg-guix check JOB=, or =borg-guix init JOB=.
